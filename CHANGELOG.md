...
- **Fixed `ImportsConditions` trait slug-prefix drift** (audit §5g — the one real bug among the 124 skipped tests). Production imports never actually set `condition_id` on `EntityCondition` rows when callers passed a `condition_name` — the trait normalized to a bare slug (e.g., `'poisoned'`) but `ConditionSeeder` stores the canonical `core:`-prefixed form (`'core:poisoned'`). Lookup returned null and the condition link was silently dropped. Trait now tries the `core:` form first, falling back to bare — same pattern as the sense lookups fixed in #236. Three `markTestSkipped` tests in `ImportsConditionsTest` rewritten with real assertions that now pass; one stale `firstOrCreate(['slug' => 'frightened'])` in `RaceImporterTest::it_imports_conditions` updated to the canonical prefix.
- **Cleaned up the 9 `markTestSkipped` sites in `FeatApiTest`** (audit §5g, first batch). Four resource-serialization tests were refactored to seed their own scenario via factories rather than hoping fixture data happened to contain a matching feat (`feat_includes_{modifiers,proficiencies,conditions,languages}_in_response`). Five redundant tests were deleted outright — `can_search_feats_by_partial_name` ("master"), `can_search_and_filter_feats_combined`, `can_paginate_search_results` ("weapon"), `can_sort_search_results` ("armor"), and `can_search_feats_case_insensitive` ("ALERT") — all had stronger coverage already provided by sibling tests (`can_search_feats_by_description`, `can_paginate_feats`, `can_sort_feats`, `can_search_feats_with_filter_only`) that use guaranteed-present data. Also corrected the `proficiencies` and `languages` resource-shape assertions that still referenced `is_choice` / `quantity` columns removed by migration `2025_01_01_000021` — those had been hidden by the skip path.
- **Cleaned up the 13 `markTestSkipped` sites in `MonsterApiTest`** (audit §5g, batch 2). Eight serialization / behavior tests were refactored to seed their own scenario via factories rather than hunting for matching fixture data (`monster_includes_{traits,actions}_in_response`, `monster_separates_legendary_actions_from_lair_actions`, `monster_without_lair_actions_returns_empty_lair_actions_array`, `can_search_monsters_by_name`, `monster_saving_throws_uses_proficient_values_when_available`, `monster_includes_legendary_metadata_in_response`, `non_legendary_monster_has_null_legendary_metadata`). Five redundant tests were deleted — `can_search_monsters_by_partial_name`, `can_search_and_filter_monsters_combined`, `can_search_monsters_by_type`, `can_paginate_search_results`, `can_sort_search_results` — all had stronger coverage in sibling tests using guaranteed data or in the dedicated `MonsterSearchTest`. The useful "MonsterSearchService intentionally does not pass sort to Meilisearch" comment from the deleted `can_sort_search_results` was preserved on `can_sort_monsters_by_name`. Net Feature-Search: 510 pass / 38 skip → 511 pass / 32 skip.
- **Cleaned up the 8 `markTestSkipped` sites in `RaceApiTest`** (audit §5g, batch 3). Seven serialization tests were refactored to seed their own scenario via factories (`it_includes_proficiencies_in_response`, `race_proficiencies_include_skill_resource`, `race_traits_with_data_tables_include_entry_resource`, `modifier_includes_skill_when_present`, `proficiency_includes_ability_score_when_present`, `race_response_includes_conditions`, `race_response_includes_spells`). Several of them also tightened sloppy `if (… ) { assert }` patterns into `assertNotNull(…)` + required assertions so they actually fail loud when the expected shape is missing. One test (`modifier_includes_choice_fields`) was deleted outright — it queried `modifiers.is_choice = true` and asserted `is_choice, choice_count, choice_constraint` fields on the response, all of which were removed by migration `2025_01_01_000021` when choice-based grants moved to `entity_choices`. Net Feature-Search: 511 pass / 32 skip → 517 pass / 30 skip.
- **Deleted the 8 `markTestSkipped` sites in `SpellEnhancedFilteringTest`** (audit §5g, batch 4). Every one was a `if (meta.total > 0) { verify each result matches filter } else { skip }` structure that duplicated coverage already provided by sibling tests in the same file using queries guaranteed to return results (via `IS EMPTY`, `level = 0`, combined class/level filters, etc.). Deleted: `it_filters_spells_by_{single,multiple}_damage_type(s)`, `it_filters_spells_by_{single,multiple}_saving_throw(s)`, `it_filters_spells_with_no_saving_throws`, `it_filters_spells_without_{verbal,somatic,material}_component`. For each filter dimension (damage types, saving throws, components), at least one non-skipping sibling test continues to verify the filter actually constrains results correctly — e.g., `it_combines_damage_type_with_level_filter` asserts fire filter returns fire spells, `it_combines_saving_throw_with_level_filter` asserts WIS filter returns WIS spells, `it_filters_subtle_spell_candidates` asserts component filters work. Net Feature-Search: 510 pass / 38 skip → 504 pass / 36 skip. Pass-count drop reflects 6 of the 8 deletions were running (and passing) on main's fixture data, but the covered filter syntax stays tested via siblings.
- **Cleaned up the `markTestSkipped` sites in `OptionalFeatureApiTest` (8) and `ClassApiTest` (7 in 5 methods)** (audit §5g, batch 5). OptionalFeatureApiTest: seven serialization tests factory-refactored to seed their own scenario (`optional_feature_includes_{sources,classes,spell_mechanics,resource_cost,prerequisites,without_prerequisites}_in_response`, `optional_feature_with_class_prerequisite_includes_class_resource`); one test (`can_sort_optional_features_by_level_requirement`) deleted as redundant with the generic `can_sort_optional_features_by_name` (the `sort_by` parameter is column-agnostic). ClassApiTest: three refactored via factory-seeded `CharacterClass + ClassFeature + ClassLevelProgression + subclass` setups; two deleted — `cleric_has_subclass_level_1` and `wizard_has_subclass_level_2` were content-validation tests asserting specific computed values for imported fixtures, with two nested skips each; the subclass_level computation logic is now tested deterministically via `base_class_includes_subclass_level`. Net Feature-Search: 510 pass / 38 skip → 513 pass / 32 skip.
- **Cleaned up `markTestSkipped` sites in `BackgroundApiTest` (3) and `SpellApiTest` (2)** (audit §5g, batch 6, small-file sweep). All five refactored to factory-seeded serialization tests: `background_traits_include_data_tables`, `background_exposes_top_level_data_tables_for_convenience`, `background_proficiencies_include_skill_resource`, `spell_includes_classes_in_response`, `spell_includes_sources_as_resource`. The backgrounds test `exposes_top_level_data_tables_for_convenience` previously had an Acolyte-specific conditional block asserting 4 specific table names — dropped as content-validation unrelated to the serialization contract being tested. Net Feature-Search: 510 pass / 38 skip → 512 pass / 36 skip.
- **Restored Scramble OpenAPI schema inference on 17 controller actions** that return `JsonResponse` to preserve `201 Created` status codes (audit §5c). Previously these endpoints generated `{ "type": "object" }` schemas in the published OpenAPI spec — now they resolve to proper `$ref: XResource` schemas while keeping the 201 status at runtime. Applied to `AuthController::register`, `CharacterController::store`, `CharacterNoteController::store`, `CharacterEquipmentController::store`, `CharacterSpellController::{store,prepare}`, `CharacterExportController::import`, `CharacterClassController::{store,levelUp,replace,setSubclass}`, `FeatureSelectionController::store`, `PartyController::{store,addCharacter}`, `PartyEncounterMonsterController::store`, `PartyEncounterPresetController::{store,load}`. **No API behavior change** — all status codes and response shapes unchanged. Fix uses Scramble's first-party `#[Response(status, type)]` attribute; rules/13 updated with the escape-hatch pattern.
- **Adopted dedicated Form Requests in 5 controller actions** that previously took the base `Illuminate\Http\Request` (audit §5d). `CharacterChoiceController::index` now uses `CharacterChoiceIndexRequest` (validates optional `type` query param), `CharacterEquipmentController::index` uses `CharacterEquipmentIndexRequest` (validates optional `exclude_currency` boolean), and `CharacterSpellController::prepare` uses `CharacterSpellPrepareRequest` (validates optional `class_slug` for multiclass). `MediaController::index` and `::destroy` had their unused `$request` parameter dropped outright — neither method read from the request body. No behavior change; validation boundary is now explicit at every endpoint.
...
- **Issue #747**: Expose `choices` field on entity view endpoints (Race, Background, Feat)
  ...
  - Added `equipment_choices` field to BackgroundResource using grouped format (matching ClassResource pattern)
  - Added `nonEquipmentChoices()` relationship method to HasEntityChoices trait
  - Enables frontend to display "You get +2 CHA" vs "Choose +1 to two abilities" on detail pages
